...
>
	...
	<div class="relative mt-2">
		<ComboboxInput
			autocomplete="off"
			class={clsx(
				'ps-3 pe-10 w-full rounded-md border-0 bg-white py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:leading-6',
				comboboxInputClass,
			)}
		...
		<ComboboxButton
			class="end-0 absolute inset-y-0 flex items-center px-2 focus:outline-none ltr:rounded-r-md rtl:rounded-l-md"
		>
			...
			>
				{#each options as option (getKey(option))}
					...
					>
						<div
							class={clsx(
								'ps-3 pe-9 relative cursor-default select-none py-2',
						...
						>
							...
							{#if selected}
								<span
									class={clsx(
										'end-0 pe-4 absolute inset-y-0 flex items-center',
