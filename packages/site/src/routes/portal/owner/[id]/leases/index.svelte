<script lang="ts" context="module">
	...
	export const load: Load = async ({ params }) => {
		...
		const { data, pagination } = await trpc.query('owner:leases:list', options);
		return {
...
<script lang="ts">
	export let leases: InferQueryOutput<`owner:leases:list`>[`data`];
	export let pagination: InferQueryOutput<`owner:leases:list`>[`pagination`];
	export let options: InferQueryInput<`owner:leases:list`>;
...
<div class="mx-auto flex max-w-4xl flex-col space-y-6 p-4 sm:p-6 lg:p-8">
	...
	<div class="">
		<LeaseList {leases} --border-radius-b="0" hideActions />
